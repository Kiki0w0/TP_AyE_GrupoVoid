...
%%	\ \	\pred{esCiudad}{ciudad: \str \times $\ent$}{$ciudad_1 \geq 50000$} 
	...
	%diagonal en ceros
	\pred{diagonalEnCeros}{ distancias: \TLista{\TLista{\ent}}}{\paraTodo[unalinea]{i}{\ent}{0 \leq i <|distancias| \implicaLuego diatancias[i][i]=0}}
	
	\pred{habitantesValidos}{c:\TLista{{\str \times \ent}}}{\paraTodo[unalinea]{i}{\ent}{0 \leq i <|c| \implicaLuego c[i] \geq 0}}
	
	% Predicado ES CAMINO
	\pred{esCamino}{distancias: \TLista{\TLista{\ent}}, camino: \TLista{\ent}, desde: {\ent}, hasta: {\ent}}{\longitud{camino} > {1} \yLuego camino[0] = desde \land camino[\longitud{camino} - 1] = hasta \land \paraTodo[unalinea]{i}{\ent}{0 \leq {i}< \longitud{camino} - 1 \implicaLuego 0 \leq camino[i], camino[i+1] < \longitud{distancias}} \land $hayConexion$(distancias,camino[i],camino[i+1])}
	
	%hayConexion
	\pred{hayConexion}{distancias:\TLista{\TLista{\ent}}, camino:\TLista{\ent}, camino[i],camino[i+1]}{\existe[unalinea]{j,k,h}{\ent}{0 \leq j,k,h < |distancias| \land (distancias[j][k]=camino[i] \land distancia[k][h]=camino[i+1]) \lor (distancias[k][j]=camino[i] \land distancias[j][h]=camino[i+1])}}
	
	
	\pred{esMatrizDeOrden1}{conexion : \TLista{\TLista{\ent}}}{
	\paraTodo[unalinea]{i,j}{\ent}{(0 \leq i,j < \longitud{conexion} \implicaLuego conexion[i][j]= conexion[j][i] \land 
			(conexion[i][j] = 1 \lor conexion[i][j] = 0))}}
	...
	\begin{enumerate} \setlength\itemsep{0cm}
		...
		\item \begin{proc}{sumaDeHabitantes}{\In menoresDeCiudades: \TLista{Ciudad}, \In mayoresDeCiudades: \TLista{Ciudad}}{\TLista{Ciudad}}
			...
			\requiere{\paraTodo[unalinea]{i}{\ent}{0 \leq {i} < \longitud{menoresDeCiudades} \implicaLuego \\
					\existe[unalinea]{j}{\ent}{0 \leq {j}< \longitud{mayoresDeCiudades}  \land menoresDeCiudades[i]_0 = mayoresDeCiudades[j]_0}}}

			\asegura{\paraTodo[unalinea]{k}{\ent}{0 \leq k < \longitud{res} \implicaLuego \existe[unalinea]{j,h}{\ent}{0 \leq j,h < \longitud{menoresDeCiudades} \land menoresDeCiudades[j]_0 = mayoresDeCiudades[h]_0 \land res[i]_1 = menoresDeCiudades[j]_1 + mayoresDeCiudades[h]_1 \land res[i]_0=mayoresDeCiudades[h]}}}
			...
			\asegura{
				(\exists conexiones : \TLista{\TLista{\TLista{\ent}}}) 
				 \paraTodo[unalinea]{i,j}{\ent}{(0 \leq i,j < \longitud{conexion}) \implicaLuego \longitud{conexiones[i]}= \longitud{C_[0]} \land 
					...
					esMultiplicacionDeMatrices(conexiones[i-1], conexiones[0], conexiones[i])} \land conexion=conexiones[n-1] }
		...
		\item \begin{proc}{caminoMinimo}{\In origen : \ent, \In destino : \ent, \In distancias : \TLista{\TLista{\ent}}}{\TLista{\ent}}
			...
			\asegura{\existe[unalinea]{camino}{\TLista{\ent}}{esCamino(distancias, camino, origen, destino) \land \paraTodo[unalinea]{camino'}{\TLista{\ent}}{esCamino(camino',distancias, origen, destino) \implicaLuego distanciaTotal(camino) \leq distanciaTotal(camino') \land res=camino}} \lor 
				\neg \existe[unalinea]{camino}{\TLista{\ent}}{esCamino(distancias, camino, origen, destino)}
